(function () {
  ...
  const ROUND_CONFIG = {
    ...
    4: { flameSpeed: 95,  fireInterval: 650,  burst: 2, enemyMoveSpeed: 42, dodgeChance: 0.93 },
    5: { flameSpeed: 110, fireInterval: 500,  burst: 2, enemyMoveSpeed: 48, dodgeChance: 0.95 },
    6: { flameSpeed: 125, fireInterval: 400,  burst: 3, enemyMoveSpeed: 55, dodgeChance: 0.97 }
  };
  ...
  function playerShoot() {
    if (state.player.jumping) return; // can only fire while standing on the ground
    const now = performance.now();
    if (now - state.lastPlayerShot < PLAYER_FIRE_COOLDOWN) return; // minimum gap between shots
  ...
  function setPauseButtonState(paused) {
    if (paused) {
      ...
      pauseLabel.textContent = 'ادامه';
    } else {
      pauseIcon.textContent = '❚❚';
      pauseLabel.textContent = 'توقف';
    }
  }
